...
- **Story size:** Each story = one Codex session (≤4h work). Larger → split via story-writer.
...
| Spike / experiment (never merges) | `spike/<slug>` | `spike/headless-Codex` |
...
- **No AI/tool attribution trailers.** Do not append `Co-Authored-By: Codex …`, `🤖 Generated with [Codex]`, or similar prefix/postfix to commit messages or PR bodies. Commits read as plain authored work.
...
The `cascade-close-story.sh` hook (in `.Codex/hooks/`) needs to be milestone-aware. Until then, run the closeout manually.
