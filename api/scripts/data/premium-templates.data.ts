/**
 * Premium canvas template seed data — the 5 format-correct designs that make up
 * the admin-curated gallery (Story, Square, A4 Print Flyer, Email Header,
 * MLS Listing Sheet).
 *
 * WHY THIS FILE EXISTS HERE, AND NOT IN `client/`
 * -----------------------------------------------
 * This data used to live in `client/src/lib/premiumTemplates.ts`, which
 * `seed-premium-templates.ts` loaded through an *optional* dynamic import.
 * That file was deleted in `216c3ef` (US-AI-037) once the initial migration had
 * run against the dev database — and because the import was optional, the seed
 * script did not fail. It silently degraded to a tag-backfill no-op.
 *
 * The consequence surfaced on 2026-09-07 as BL-25: production and staging both
 * had **zero** admin-curated canvas templates, so the first screen a new user
 * saw after registering read "No templates found matching your criteria." The
 * documented remedy — "run seed-premium-templates.ts, then
 * update-template-images.ts" — could never have worked: with the source module
 * gone the first script created nothing, and the second only `update()`s rows
 * that already exist.
 *
 * Recovered verbatim from `216c3ef^:client/src/lib/premiumTemplates.ts`. The
 * only edits are structural, not content:
 *   - `import type { StarterCanvasTemplate }` (a client-side, type-only import)
 *     is inlined below, so this module has no imports at all and cannot be
 *     broken by a client refactor or deletion again.
 *   - The unused `PREMIUM_BY_ID` lookup and `getPremiumCanvasTemplateById()`
 *     helper are dropped — the seed script only ever consumed the array.
 *
 * The seed script now imports this **statically**. If this file ever goes
 * missing the script fails to start, loudly, instead of seeding nothing.
 *
 * Thumbnails here are inline SVG data-URLs. `update-template-images.ts` swaps
 * them for real photography after seeding; that script is the second half of
 * the procedure, not an alternative to it.
 */
