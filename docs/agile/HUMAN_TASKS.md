...
| 3 | Phase 1 | EPIC-LAUNCH-01 | US-LAUNCH-005 AC5 | Run `npm run verify:payment-prereqs` against production config | ✅ **Done 2026-09-03 — passes (exit 0), all 10 vars present.** ⚠️ But it reports `RAZORPAY_KEY_ID` and `VITE_RAZORPAY_KEY_ID` as **test keys** (`rzp_test_` prefix). With `BETA_MODE` unset and `/pricing` publicly returning 200, production can be reached for checkout in Razorpay **test mode**. No real money can be taken by accident; equally, no real subscription can succeed. **This makes task #4 blocked, not merely unscheduled.** |
| 4 | Phase 1 | EPIC-LAUNCH-01 | US-LAUNCH-005 AC6 | One real ₹ subscription on production (smallest plan) → verify webhook activates it → refund/cancel from dashboard | ⛔ **BLOCKED (found 2026-09-03), not just held.** Production runs Razorpay **test** keys — a real card cannot be charged there today. Order is: #6 live plans → set live keys on production → then this. Still needs your explicit go-ahead |
...
| 6 | Phase 1 | EPIC-PAY-05 (V1) | US-PAY-109 T0 | Create **8** Razorpay Plan objects per tier×interval — see §6 for the exact amounts (annual figures corrected 2026-08-27: they were stale ×10-derived values, superseded by `PLAN_CONFIG`'s authored prices). Naming: `BG-<TIER>-<INTERVAL>-<YYYY>-<MM>`. **Amount field takes rupees, not paise.** | ✅ **DONE — live mode verified 2026-09-08.** All 8 LIVE Plan objects exist on production with the exact `PLAN_CONFIG` amounts and correct cadences, and none is one of the abandoned 100× objects below. Staging is equally complete on TEST-mode plans. Verified by `npm run audit:razorpay-plans`, not by eye. The task had in fact been completed in the dashboard and was never closed out here — US-PAY-109 carried "blocked on T0 HUMAN task" while the plans were live. |
...
**This class of error is now caught automatically.** `npm run audit:razorpay-plans` (`api/scripts/audit-razorpay-plans.ts`, added 2026-09-08) fetches every configured Plan from the Razorpay API and compares its real amount against `PLAN_CONFIG`, failing the run on any mismatch. A 100× plan wired into an env var would have been caught the first time it was checked, rather than depending on someone noticing. It also guards the billing cadence, since a correct amount on the wrong period looks right on the pricing page.
