...
| TC-LAUNCH-010-05 | Manual | P0 | Given staging's existing var set on a real deploy, When the branch deploys to staging, Then logs show `Nest application successfully started` and `/api/health` = ok (AC4). | 🔲 | **Cannot run until merged** — staging auto-deploys on push to `main`. Do this after merge, before closing the story |
...
- [ ] Staging deploy boots cleanly with no staging variable change (AC4) — **only remaining implementation-side gate; requires merge**
- [ ] PR merged (PR #{number})
