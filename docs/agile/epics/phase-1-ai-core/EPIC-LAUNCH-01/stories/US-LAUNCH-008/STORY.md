...
- [x] **AC1 [documentation]:** Policy documented in `docs/agile/PROJECT_CONTEXT.md` (Plan Tiers section) and CLAUDE.md plan-tier line: plan limits count **generations** (user-facing unit), `creditsUsed = 1` per generation; `costUsd` records true provider spend for margin analytics — the two are intentionally different numbers
  > **Amended 2026-08-15 (US-LAUNCH-015):** editable-design composes are metered as a distinct case, not covered when this AC was written. The first distinct variation compose per generation is free on paid tiers; each *additional* distinct variation composed on the same generation also consumes 1 credit (same `creditsUsed` unit, incremented alongside the existing `costUsd` write in `AiOrchestrator.composeDesignForEdit`). FREE tier gets exactly one lifetime trial compose, then a 402 `EDITABLE_REQUIRES_UPGRADE` rather than a credit charge — it has no credits to spend on this feature. Cache hits (US-AI-048) never gate and never meter, on any tier. CLAUDE.md's metering-policy line carries the current, amended wording.
