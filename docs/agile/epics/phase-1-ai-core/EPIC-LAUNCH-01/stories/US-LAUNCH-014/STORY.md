...
> "Acceptance (Done When)" checklist — that list is unchanged — but it should not be treated as optional filler when
> sequencing. **Deploy note:** `prisma db push` must reach an environment *before* this code does, or sign-up breaks there.
...
- [x] **AC17 [happy-path]:** `.env.example` and the epic's `ENV.yaml` document `INTERNAL_TEST_EMAIL_DOMAINS`
...
- [x] **AC19 [rate-limit]:** Internal-test traffic is exempt from the **global** `100/min` cap, not only from the
...
|-------|------|----------|----------|--------|---------|
| MV-014-01 | Manual | P0 | Register with a real 10minutemail / mailinator address on localhost → friendly "permanent email" error; no Organization, User or token row created (AC2) | ✅ | **Verified 2026-09-21.** `mailinator.com`, `10minutemail.com` and `guerrillamail.com` each returned `400 {"code":"DISPOSABLE_EMAIL_NOT_ALLOWED"}` with the friendly message. Confirmed **no rows written**: a direct DB count for all three domains returns 0 users, so the check fires before any Organization/User/token insert. |
| MV-014-02 | Manual | P0 | Register `me+test@gmail.com` after `me@gmail.com` exists (both created under this story) → 409 (AC4) | ✅ | **Verified 2026-09-21.** Base `bldgt.alias…@gmail.com` → `201`; the `+promo` alias → `409 User already exists`; the dotted variant `b.l.d.g.t.alias…@gmail.com` → `409`. Both Gmail normalization rules (plus-tag and dots) hold against the live API. |
| MV-014-03 | Manual | P0 | New unverified user can log in, onboard, browse templates and open the editor; clicking Generate → "Verify your email" dialog, usage count unchanged, no provider call in API logs (AC8/AC9) | ✅ | **API half 2026-09-19:** both gated routes → `403 {"code":"EMAIL_NOT_VERIFIED"}`, which also proves T5b end-to-end. **Browser half 2026-09-21:** banner renders for an unverified user and fits 375px (367 ≤ 367); the dialog opens and fits (367). *Scope note:* the dialog was opened by dispatching the event directly, so it proves the dialog + listener render and wire up, **not** that `queryClient` dispatches on a live 403 — that path is unit-tested and the 403 itself was verified against the running API. |
| MV-014-04 | Manual | P0 | Open the `[DEV EMAIL]` verify link in the same browser → success state; banner disappears without re-login; Generate now works (AC6/AC10) | ✅ | **API half 2026-09-19:** `200 {verified:true,userId}`; replay → `400` (single-use holds); `login()` then returns `emailVerified: true`. **Browser half 2026-09-21:** success state renders, `localStorage.auth_user.emailVerified` flips to `true` with no re-login, banner gone on `/templates`, no overflow at 375px. **This check found a real bug and it is fixed:** the page hung on "Verifying your email…" forever while the account verified server-side — `useEffect` depended on `login`, which `AuthProvider` re-creates each render, so a mid-flight re-render cancelled the only state update while `startedRef` blocked any retry. `login` is now held in a ref and the effect depends on `[token]`. Not a StrictMode artifact (`main.tsx` has none) — it would have hit real users on the happy path. |
...
| MV-014-07 | Manual | P1 | Compose ("Make Editable") and Regenerate as an unverified user → 403 dialog, no credit charged (AC8) | ✅ | **Verified 2026-09-21.** Both `POST /infographics/generations/:id/compose` and `…/regenerate` returned `403 {"code":"EMAIL_NOT_VERIFIED"}` for an unverified session. The id used was deliberately non-existent and still produced 403, proving the guard runs before the record is resolved — no credit, no provider call. |
| MV-014-08 | Manual | P2 | `RESEND_API_KEY` set + real inbox → verification email arrives, link verifies, generation unlocks (AC5) | ✅ | **Verified 2026-09-23** against the real provider. Registered `din.prajapati+bldgverify…@gmail.com` on a local API carrying staging's `RESEND_API_KEY`; no `[DEV EMAIL]` fallback fired, so it was a genuine send. Mail arrived ~2s later in **INBOX (not spam)** from `noreply@buildographic.com`, subject "Verify your email address", body carrying `/auth/verify-email?token=…` and the 24-hour notice. Following the link → `200 {verified:true}`; re-login → `emailVerified: True`; the gated route no longer 403s. **Two notes:** the link host came from the local `CLIENT_URL` (`localhost:5000`) — **check the host in the first real email after deploy**, as no test covers that. And the alias registered rather than 409'ing, because the pre-existing `din.prajapati@gmail.com` row has `emailNormalized` NULL — the documented out-of-scope gap, now observed live. |
| MV-014-09 | Manual | P2 | Banner at 375px and VerifyEmailPage at 375/1440px — no horizontal overflow (AC10) | ✅ | **Verified 2026-09-21.** This story's elements fit: banner `width 367 ≤ vw 367`; dialog `367`; VerifyEmailPage `scrollWidth == clientWidth` at both 375 and 1440. **Separate pre-existing defect found:** `/templates` itself overflows 100px at 375px (`scrollWidth 467` vs `clientWidth 367`) — `AppHeader`'s nav row, **not** the banner: identical figures and the same 7 offending elements with the banner shown and hidden. Filed as [[BL-32]]. |
| MV-014-10 | Manual | P0 | With `INTERNAL_TEST_EMAIL_DOMAINS=test.local` on the local API, a full `npx playwright test` against localhost completes with no 403 from the gate and no 429 from the sign-up limit (AC17) | ✅ | **Verified 2026-09-23 for the 24 non-AI specs: 184 passed, 0 failed, 41 skipped, 15 min** — a single uninterrupted run (an earlier attempt was killed by memory at 79 tests and is NOT counted). Zero occurrences of `EMAIL_NOT_VERIFIED`, `Too Many Requests` or `DISPOSABLE_EMAIL` in the output, which is the claim this item exists to test. **11 specs excluded deliberately on cost** — `us-ai-031/032-editable/032-export/048/049/050/051/053`, `us-edit-005/009`, `us-launch-015` all reach live generation or compose; measured spend from `UsageRecord` is **$0.1495 average, $0.274 max per charged op**, so a full run is roughly **$1–3**. Excluded, not failed: the allowlist claim is proven across the other 24. |
...
| MV-014-11 | Manual | P0 | **Security:** with the variable UNSET, registering `a@test.local` behaves like any other address — unverified, token created, email sent, throttled after 5/h (AC14) | ✅ | **Verified 2026-09-21** on a dev server booted with `INTERNAL_TEST_EMAIL_DOMAINS` unset: registering `@test.local` returned `emailVerified = false` — no bypass. Confirms the feature is inert unless deliberately enabled, which is what keeps production safe. The 5/h cap was separately observed in the same session (registrations refused with `429` once the quota was spent). |
...
| MV-014-14 | Manual | P0 | **Production policy:** confirm `INTERNAL_TEST_EMAIL_DOMAINS` is absent from the Railway production environment before/after deploy (AC14) | ⚠️ | **Pre-deploy check passed 2026-09-21:** `railway variables --environment production` (53 vars) does not contain `INTERNAL_TEST_EMAIL_DOMAINS`; it is absent from staging too. **Must be re-run AFTER the deploy** — that is when it actually protects anything — so this stays open until then. |
