...
| MV-014-15 | Auto (E2E) | P1 | Targeted Playwright run against localhost shows no regression on the auth surface this story touched | ✅ | **Verified 2026-09-19** with `PLAYWRIGHT_BASE_URL=http://localhost:5000` (the default points at staging — must be overridden). `us-launch-003-password-reset` 4/4 passed, incl. the forgot-password form now under the 5/h cap. `us-ai-040` 5/5 passed. An earlier run of the same specs failed 5 tests purely from **memory starvation** (0.46 GB free, ~4 min/test); re-run at 3.95 GB free passed all five in 24.3s — environment, not defect. The 12 register-then-generate specs were excluded on purpose: they spend real GPT-4o/Ideogram credit. |
