...
| P-24 | **Source maps** resolve: Sentry shows readable filenames, not bundle hashes                      | ⚠️     | **Inconclusive from this test, not a confirmed fail.** The only non-`<anonymous>` stack frame belongs to Sentry SDK's own bundled wrapper (`browserapierrors.setTimeout`), which showed raw minified code (`/assets/index-C-Izxx-r.js:33:9490 in n`) — but that's third-party SDK code, not app code, and may not even have source maps uploaded for it. My injected `throw` doesn't pass through real app/React code, so there was no genuine app-code frame to test resolution against. **To properly verify:** trigger an error from inside real app code (e.g. a deliberately broken dev-only route, or catch a real runtime error during normal use) and check whether *that* frame resolves to a real filename. `SENTRY_AUTH_TOKEN` is already set per the 2026-07-24 session log — build-time upload should be wired, but hasn't been visually confirmed in a real app-code stack trace yet |
