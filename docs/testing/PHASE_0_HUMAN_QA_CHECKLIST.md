...
| 3   | [Task 3 — Production](#task-3--production-go-live--sentry-verify)                                 | Live keys, v1.0.0 tag, Sentry, Google OAuth prod                     | 🟡 **IN PROGRESS** — domain live + verified, Neon/Railway env vars reconciled 2026-07-25 (3A/3B ✅, found+fixed a real `BASE_URL`/`CLIENT_URL` bug on both envs); reset-link fix re-verified live against production 2026-07-21 (real email, real domain, full reset flow). 🔴 **New blocker found 2026-07-25:** US-LAUNCH-010's merged RazorPay guard will abort production boot on the next `v1.0.0` tag deploy (production's TEST keys vs. the guard's LIVE-key requirement) — resolve before P-10 (see 3C). Remaining: resolve that conflict, confirm Prod OAuth client (3F), tag `v1.0.0` (3C), full smoke test (3D — P-13/P-14 partially done; P-18/19 live-checkout rows are M-LAUNCH-02 scope, not needed here). Sentry (3E): P-21/22/23 confirmed live 2026-07-25; P-24 (source-map resolution) root-caused 2026-07-26 — not a code bug, `SENTRY_AUTH_TOKEN` lacks the `project:releases` scope (403 on `sentry-cli releases new`, found in Railway build logs). Release-name-matching code fix already applied and correct; needs a new-scoped token in Sentry + Railway (staging & production), then re-test |
...
| P-24 | **Source maps** resolve: Sentry shows readable filenames, not bundle hashes                      | ❌     | **Root cause confirmed 2026-07-26 — Sentry auth token lacks permission, not a code bug.** Added an explicit, matching `release` (short git SHA, `RAILWAY_GIT_COMMIT_SHA`) to both `Sentry.init()` (`client/src/main.tsx`, `api/src/instrument.ts`) and `sentryVitePlugin()` (`vite.config.ts`) — a real fix for the release-mismatch pitfall, but re-testing against staging (`BUILDOGRAPHIC-7`) still showed a raw minified frame. Checked Railway's **build** logs (`railway logs --environment staging --build`) directly and found the actual error: `sentry-cli releases new {sha}` → `403 You do not have permission to perform this action`. The upload pipeline fails at the release-creation step, before source maps are ever associated with anything. **Action needed (Sentry dashboard, not code):** the `SENTRY_AUTH_TOKEN` set in Railway needs the `project:releases` scope (per `.env.example`'s own comment) — generate a new token with that scope in Sentry → Settings → Auth Tokens and update it on both staging and production Railway environments, then re-test with a real app-code error. The release-name fix above is still correct and should be kept regardless. |
