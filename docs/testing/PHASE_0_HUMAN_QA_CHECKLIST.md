...
| P-24 | **Source maps** resolve: Sentry shows readable filenames, not bundle hashes                      | ❌     | **Confirmed failing, 2026-07-26.** Added a temporary dev-only route (`/dev/sentry-test-temp`, deleted after this test) with a button whose `onClick` throws real app code. Event `BUILDOGRAPHIC-6` on staging still showed the raw minified frame `/assets/index-BRChQA1D.js:926:140514 in onClick` with mangled variable names — not `SentryTestPage.tsx`. The build does emit `//# sourceMappingURL=index-BRChQA1D.js.map`, and `SENTRY_AUTH_TOKEN`/`SENTRY_ORG`/`SENTRY_PROJECT` are confirmed present on the staging Railway environment (checked via `railway variables`, values not read) — so the upload step likely isn't the failure. **Likely root cause:** neither `client/src/main.tsx`'s `Sentry.init()` nor the `sentryVitePlugin()` config in `vite.config.ts` sets an explicit `release` identifier. `@sentry/vite-plugin` auto-generates a release name when it uploads maps, but if the browser SDK doesn't report that same release string at runtime, Sentry can't match a live event to its uploaded map — a known Sentry+Vite pitfall. Not yet fixed — needs explicit, matching `release` config on both sides, then a re-test. |
