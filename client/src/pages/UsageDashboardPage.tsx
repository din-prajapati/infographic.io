    ...
    <div className="min-h-screen" style={{ background: 'var(--page-bg)' }}>
      ...
      <div className="border-b border-border glass">
        <div className="container flex h-14 items-center justify-between px-6 max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" className="border-border text-foreground hover:bg-accent" onClick={() => handleExport('csv')}>
              <Download className="h-4 w-4 mr-2" />
              Export CSV
            </Button>
            <Button variant="outline" size="sm" className="border-border text-foreground hover:bg-accent" onClick={() => handleExport('json')}>
              <Download className="h-4 w-4 mr-2" />
              Export JSON
            </Button>
          </div>
      ...
      <main className="container px-6 py-8 max-w-6xl mx-auto">
          ...
          <div className="glass rounded-xl border border-border p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-foreground">{totalInfographics}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select 
                    ...
                    className="text-sm border rounded px-2 py-1 bg-input-background border-border text-foreground"
                  >
                ...
                <div className="space-y-3">
                  {monthlyUsage.map((item, index) => (
                    <div key={index} className="flex items-center gap-4">
                      <div className="w-20 text-sm text-muted-foreground shrink-0">{item.month}</div>
                      <div className="flex-1">
                        <div className="h-8 bg-accent rounded-full overflow-hidden">
                          <div 
                      ...
                      <div className="w-16 text-right text-sm font-medium text-foreground">{item.count}</div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="glass overflow-hidden rounded-xl border border-border">
            <div className="border-b border-border px-6 py-4">
              <h3 className="font-semibold text-foreground">Cost Breakdown by AI Model</h3>
              <p className="mt-1 text-sm text-muted-foreground">Usage and cost per generation model</p>
            </div>
            <div className="p-6">
              {breakdownLoading ? (
                <div className="flex items-center justify-center h-48">
                  ...
                  {costBreakdown.map((item, index) => (
                    <div key={index} className="p-4 bg-accent/50 rounded-lg">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">Total: ${item.totalCostUsd.toFixed(2)}</span>
                        <span className="text-muted-foreground">Avg: ${item.averageCostUsd.toFixed(4)}/gen</span>
                      </div>
                    </div>
                  ))}
                </div>
